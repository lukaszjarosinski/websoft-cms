...
class FormValidator 
{
	...
	function addValidation($variable,$validator,$error)
	{
		...
		$validator_obj->variable_name = str_replace(" ","_",$variable);
	...
	function ValidateForm()
	{
		...
		//print_r($form_variables);

        $vcount = count(array($this->validator_array));
		...
		foreach($this->validator_array as $val_obj)
		{
			...
			{
				$bret = false;
                $this->error_hash[str_replace("_"," ",$val_obj->variable_name)] = $error_string;
			}
		}

		if(true == $bret && count(array($this->custom_validators)) > 0)
		{
		  ...
		  case "selmin":
						{
							...
							if(isset($input_value))
                            {
							    if($min_count > 1)
							    {
							        $bret = (count(array($input_value)) >= $min_count )?true:false;
							    }
                                else
                                {
                                  $bret = true;
