...
if ($zalogowano)
{
	if (isset($_REQUEST['action']) AND !empty($_SERVER['HTTP_X_REQUESTED_WITH']) AND strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
	{
		switch($_REQUEST['action'])
		{
			case "add":
				...
				{
					$return = $posts->addEditPost($_REQUEST);
	...
	//numer strony - dotyczy paginacji wpisów
	if (isset($_REQUEST['page'])) $page = $_REQUEST['page'];
	else $page = '';
	//numer kategorii - dotyczy filtrowania wpisów
	if (isset($_REQUEST['category'])) $category = $_REQUEST['category'];
	else $category = '';
	if (isset($_REQUEST['subaction']) AND ($_REQUEST['subaction'] == 'up' OR $_REQUEST['subaction'] == 'down'))
	{
		$id = $posts->filterData($_REQUEST['id']);
		$posts->order($id,$posts->filterData($_REQUEST['subaction']));
	}
	if (isset($_REQUEST['subaction']) AND $_REQUEST['subaction'] == 'position')
	{
		$id = $posts->filterData($_REQUEST['id']);
		$position = $posts->filterData($_REQUEST['position']);
		$posts->position($id,$position);
	}
	$smarty->assign(
		array(
	...
	$smarty->display('post'.((isset($_REQUEST['action']) AND $_REQUEST['action'] != '')?'_'.$_REQUEST['action']:'').'.tpl');
}
