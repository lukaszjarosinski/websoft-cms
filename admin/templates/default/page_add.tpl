...
<p><label for="text1">{$lang.text}</label><br /><textarea name="text1" id="text1" rows="7" cols="30" class="text-input textarea">{$post.text}</textarea></p>
...
<p><label for="parent">{$lang.parent}</label> <select name="parent" id="parent" class="medium-input"><option value="">--{$lang.select}--</option>
{foreach from=$posts->parentPagesList($post.id) item=a}
{assign var=level value=0}
<option value="{$a.id}"{if $a.id == $post.parent} selected="selected"{/if}>{$a.title}</option>
{if $a.submenu}
...
<p><label for="order">{$lang.order}</label> <input type="text" class="text-input small-input" name="order" id="order" value="{$posts->getMaxOrder('page')}" /></p>
<p><label for="post_on">{$lang.post_on}</label> <input type="radio" name="post_on" id="post_on1" value="1"{if $post.post_on == '1'} checked="checked"{elseif !$post.post_on} checked="checked"{/if} /> {$lang.yes} <input type="radio" name="post_on" id="post_on2" value="0"{if $post.post_on && $post.post_on == '0'} checked="checked"{/if} /> {$lang.no} </p>
...
{foreach from=$posts->pagesList('',false) item=pages_list}
...
wartosc = ($.ajax({
url: "{/literal}{$domain}{literal}admin/page.php?action=slugExists",
async: false,
data: 'slug='+value
}));
...
formSubmit('page_add','{/literal}{$domain}{literal}admin/page.php','{/literal}{$domain}{literal}admin/page.php');
},
rules: {
title: {
required:true,
