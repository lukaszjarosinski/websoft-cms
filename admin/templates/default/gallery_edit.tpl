...
<h3>{$lang.gallery_pictures_edit}</h3>
...
<p><label for="position">{$lang.position}</label> <input type="text" name="position" id="position" class="text-input small-input" value="{$picture.position}" /></p>
...
{if ($picture.image <> '')}
{assign var=big_image value="`$gallery_dir``$picture.image`"}
{assign var=min_image value="`$gallery_dir``$min_prefix``$picture.image`"}
...
{if file_exists($image)}<p><label>Aktualne zdjęcie</label> <img src="{$domain}images/gallery/{if file_exists($min_image)}{$min_prefix}{/if}{$picture.image}" alt="" /></p>
<p><span>Usuń zdjęcie</span> <input type="checkbox" name="delete_picture" id="delete_picture" value="1" /></p>
{/if}
{/if}
