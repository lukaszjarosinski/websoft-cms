...
<option value="{$a.id}"{if $smarty.request.action == 'edit' AND in_array($a.id,$post.categories)} selected="selected"{/if}>{$a.title}</option>
{if $a.submenu}
...
<p><label for="order">{$lang.order}</label> <input type="text" class="text-input large-input" name="order" id="order" value="{$posts->getMaxOrder('post')}" /></p>
