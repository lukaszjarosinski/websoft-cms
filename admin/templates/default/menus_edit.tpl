...
<option value="{$a.id}"{if $smarty.request.action == 'edit' AND in_array($a.id,$menu.array) AND is_array($menu.array)} selected="selected"{/if}>{$a.title}</option>
{if $a.submenu}
...
formSubmit('menu_add','{/literal}{$domain}{literal}admin/menus.php');
},
rules: {
title: {
required:true,
