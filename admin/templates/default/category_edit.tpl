...
{foreach from=$posts->parentCategoriesList($post.id) item=a}
...
<input type="hidden" name="status" value="{if $post.status != ''}{$post.status}{else}published{/if}" />
...
wartosc = ($.ajax({
url: "{/literal}{$domain}{literal}admin/category.php?action=slugExists",
async: false,
